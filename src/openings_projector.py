...
def initial_triangulation_1view_raycasting(im1, P, R, t, K, k_dist, P_type, poses, i, images_path, keypoints_path, how2get_kp, opening_4points, data_folder, pp, plane_params=None, dom=None):
    ...
    if len(x1.T)>0:
        ...
        if plane_params is not None: 
            ...
            dom.LOD2.mesh.compute_vertex_normals() #! 
            ...
            #Checking if the rays did not hit the obj. If they didnt, the dist2int of those points are the mean of the others 
            ...
            for jj in range(int(len(rays_list)/4)):
                ...
                plane_id_hit = dom.LOD2.plane_clusters[0][op_hit[np.where(op_dist!=np.inf)]] #planes that are hit by the rays #TODO copy this for cracks projection to just project cracks once??
...
def op_projector(DADT, data_folder, images_path, keypoints_path, intrinsic, poses, how2get_kp, im1, opening_4points, pp=None, ctes=[.05, .8, .1, .3]):
    """
    ...
    #Regularize openings paper 2
    DADT.regularize_openings(ctes)
    #Regularize following LOD2 regularization transformation. 
    DADT.regularize_openings_init() #!tune the threshold or give it as input
    #Save openings
